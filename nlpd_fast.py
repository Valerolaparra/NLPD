...
def conv2d_grouped(image: np.ndarray, kernel: np.ndarray, stride: int = 1, 
    ...
    if groups == C:
        ...
        output_list = []
        for c in range(C):
            # Convolve channel with its corresponding filter
            conv_result = convolve(image[c], kernel[c, 0], mode='constant', cval=0.0)
            # Apply stride by slicing
            if stride > 1:
                conv_result = conv_result[::stride, ::stride]
            output_list.append(conv_result)
        output = np.stack(output_list, axis=0)
    else:
        # Standard grouped convolution (slower path, but rarely used)
        H_out = (H - kH) // stride + 1
        W_out = (W - kW) // stride + 1
        ...
        for g in range(groups):
            for c in range(channels_per_group):
                ...
                for in_c in range(channels_per_group):
                    ...
                    conv_result = convolve(image[in_channel], kernel[out_c, in_c], 
...
def interpolate_bilinear(image: np.ndarray, target_size: List[int]) -> np.ndarray:
    """
    Bilinear interpolation to resize image. Optimized version.
    ...
    # Use scipy's zoom with order=1 for bilinear, much faster than manual implementation
    return zoom(image, zoom_factors, order=1, mode='nearest')
